...
import { StatCard, SectionTitle } from '../../components/bento'
  ...
  const loadHistory = useCallback(async () => {
    ...
    try {
      ...
      const queries: BatchQueryItem[] = [
        ...
        { id: 'topicPartitions', query: q('topicPartitions', `sum(kafka_topic_partitions{app="kmanager",cluster_id="${clusterId}",topic!~"__.*"})`), start: instantStart, end: instantEnd, step: '60s' },
        ...
        ...(jmxAvailable ? [
          ...
          { id: 'fencedBroker', query: q('fencedBroker', `max(kafka_controller_kafkacontroller_fencedbrokercount{app="kmanager",cluster_id="${clusterId}"})`), start: instantStart, end: instantEnd, step: '60s' },
          { id: 'globalPartition', query: q('globalPartition', `max(kafka_controller_kafkacontroller_globalpartitioncount{app="kmanager",cluster_id="${clusterId}"})`), start: instantStart, end: instantEnd, step: '60s' },
          { id: 'globalTopic', query: q('globalTopic', `max(kafka_controller_kafkacontroller_globaltopiccount{app="kmanager",cluster_id="${clusterId}"})`), start: instantStart, end: instantEnd, step: '60s' },
          { id: 'replicaImbalance', query: q('replicaImbalance', `max(kafka_controller_kafkacontroller_preferredreplicaimbalancecount{app="kmanager",cluster_id="${clusterId}"})`), start: instantStart, end: instantEnd, step: '60s' },
          { id: 'offlinePartitions', query: q('offlinePartitions', `max(kafka_controller_kafkacontroller_offlinepartitionscount{app="kmanager",cluster_id="${clusterId}"})`), start: instantStart, end: instantEnd, step: '60s' },
          { id: 'activeController', query: q('activeController', `max(kafka_controller_kafkacontroller_activecontrollercount{app="kmanager",cluster_id="${clusterId}"})`), start: instantStart, end: instantEnd, step: '60s' },
          { id: 'offlineLogDirCount', query: q('offlineLogDirCount', `max(kafka_log_logmanager_offlinelogdirectorycount{app="kmanager",cluster_id="${clusterId}"})`), start: instantStart, end: instantEnd, step: '60s' },
          { id: 'logDirOffline', query: q('logDirOffline', `max(kafka_log_logmanager_logdirectoryoffline{app="kmanager",cluster_id="${clusterId}"})`), start: instantStart, end: instantEnd, step: '60s' },
          { id: 'invalidMagic', query: q('invalidMagic', `sum(kafka_server_brokertopicmetrics_invalidmagicnumberrecords_total{app="kmanager",cluster_id="${clusterId}"})`), start: instantStart, end: instantEnd, step: '60s' },
          { id: 'invalidCrc', query: q('invalidCrc', `sum(kafka_server_brokertopicmetrics_invalidmessagecrcrecords_total{app="kmanager",cluster_id="${clusterId}"})`), start: instantStart, end: instantEnd, step: '60s' },
          { id: 'invalidOffset', query: q('invalidOffset', `sum(kafka_server_brokertopicmetrics_invalidoffsetorsequencerecords_total{app="kmanager",cluster_id="${clusterId}"})`), start: instantStart, end: instantEnd, step: '60s' },
        ] : []),
        // 范围查询：AdminClient 指标（始终查询）
        { id: 'produceRate', query: q('produceRate', `sum(rate(kafka_topic_partition_current_offset{app="kmanager",cluster_id="${clusterId}",topic!~"__.*"}[30s]))`), start: start.unix(), end: end.unix(), step },
        { id: 'consumeRate', query: q('consumeRate', `sum(rate(kafka_consumergroup_current_offset{app="kmanager",cluster_id="${clusterId}"}[30s]))`), start: start.unix(), end: end.unix(), step },
        { id: 'lagTrend', query: q('lagTrend', `sum(kafka_consumergroup_lag_sum{app="kmanager",cluster_id="${clusterId}"})`), start: start.unix(), end: end.unix(), step },
        // 范围查询：JMX 指标（仅 JMX Exporter 可用时才查）
        ...(jmxAvailable ? [
          { id: 'bytesIn', query: q('bytesIn', `sum(rate(kafka_server_brokertopicmetrics_bytesin_total{app="kmanager",cluster_id="${clusterId}"}[30s]))`), start: start.unix(), end: end.unix(), step },
          { id: 'bytesOut', query: q('bytesOut', `sum(rate(kafka_server_brokertopicmetrics_bytesout_total{app="kmanager",cluster_id="${clusterId}"}[30s]))`), start: start.unix(), end: end.unix(), step },
          { id: 'bytesRejected', query: q('bytesRejected', `sum(rate(kafka_server_brokertopicmetrics_bytesrejected_total{app="kmanager",cluster_id="${clusterId}"}[30s]))`), start: start.unix(), end: end.unix(), step },
          { id: 'failedProduce', query: q('failedProduce', `sum(rate(kafka_server_brokertopicmetrics_failedproducerequests_total{app="kmanager",cluster_id="${clusterId}"}[30s]))`), start: start.unix(), end: end.unix(), step },
          { id: 'failedFetch', query: q('failedFetch', `sum(rate(kafka_server_brokertopicmetrics_failedfetchrequests_total{app="kmanager",cluster_id="${clusterId}"}[30s]))`), start: start.unix(), end: end.unix(), step },
          { id: 'messagesIn', query: q('messagesIn', `sum(rate(kafka_server_brokertopicmetrics_messagesin_total{app="kmanager",cluster_id="${clusterId}"}[30s]))`), start: start.unix(), end: end.unix(), step },
        ] : []),
      ]
    ...
    <>
      ...
      <DashboardGrid
        ...
        items={[
          ...
          { i: 'lag-chart', x: 0, y: 10, w: 12, h: 6, component: <div className="bento-card"><div className="bento-card-inner"><SectionTitle title="消费者组总 Lag" /><EChartsReact echarts={echarts} key={`lag-${cluster?.cluster_id}-${quickRange}`} option={buildLineChartOption('', lagTrendData, '#ef4444')} style={{ height: 250 }} notMerge={true} /></div></div> },
          { i: 'produce-rate', x: 0, y: 16, w: 6, h: 6, component: <div className="bento-card"><div className="bento-card-inner"><SectionTitle title="集群生产速率" /><EChartsReact echarts={echarts} key={`pr-${cluster?.cluster_id}-${quickRange}`} option={buildLineChartOption('', produceRateData, '#3b82f6', 'msg/s')} style={{ height: 250 }} notMerge={true} /></div></div> },
          { i: 'consume-rate', x: 6, y: 16, w: 6, h: 6, component: <div className="bento-card"><div className="bento-card-inner"><SectionTitle title="集群消费速率" /><EChartsReact echarts={echarts} key={`cr-${cluster?.cluster_id}-${quickRange}`} option={buildLineChartOption('', consumeRateData, '#10b981', 'msg/s')} style={{ height: 250 }} notMerge={true} /></div></div> },
          // JMX 趋势图（仅 JMX Exporter 可用时显示）
          ...(jmxAvailable ? [
          { i: 'bytes-in', x: 0, y: 22, w: 6, h: 6, component: <div className="bento-card"><div className="bento-card-inner"><SectionTitle title="字节流入速率" /><EChartsReact echarts={echarts} key={`bi-${cluster?.cluster_id}-${quickRange}`} option={buildLineChartOption('', bytesInData, '#10b981', 'bytes/s', formatBytesForChart)} style={{ height: 250 }} notMerge={true} /></div></div> },
          { i: 'bytes-out', x: 6, y: 22, w: 6, h: 6, component: <div className="bento-card"><div className="bento-card-inner"><SectionTitle title="字节流出速率" /><EChartsReact echarts={echarts} key={`bo-${cluster?.cluster_id}-${quickRange}`} option={buildLineChartOption('', bytesOutData, '#f59e0b', 'bytes/s', formatBytesForChart)} style={{ height: 250 }} notMerge={true} /></div></div> },
          { i: 'messages-in', x: 0, y: 28, w: 6, h: 6, component: <div className="bento-card"><div className="bento-card-inner"><SectionTitle title="消息流入速率" /><EChartsReact echarts={echarts} key={`mi-${cluster?.cluster_id}-${quickRange}`} option={buildLineChartOption('', messagesInRateData, '#8b5cf6', 'msg/s')} style={{ height: 250 }} notMerge={true} /></div></div> },
          { i: 'bytes-rejected', x: 6, y: 28, w: 6, h: 6, component: <div className="bento-card"><div className="bento-card-inner"><SectionTitle title="拒绝字节速率" /><EChartsReact echarts={echarts} key={`br-${cluster?.cluster_id}-${quickRange}`} option={buildLineChartOption('', bytesRejectedData, '#ef4444', 'bytes/s', formatBytesForChart)} style={{ height: 250 }} notMerge={true} /></div></div> },
          { i: 'failed-produce', x: 0, y: 34, w: 6, h: 6, component: <div className="bento-card"><div className="bento-card-inner"><SectionTitle title="生产请求失败率" /><EChartsReact echarts={echarts} key={`fp-${cluster?.cluster_id}-${quickRange}`} option={buildLineChartOption('', failedProduceRateData, '#ef4444', '次/秒')} style={{ height: 250 }} notMerge={true} /></div></div> },
          { i: 'failed-fetch', x: 6, y: 34, w: 6, h: 6, component: <div className="bento-card"><div className="bento-card-inner"><SectionTitle title="拉取请求失败率" /><EChartsReact echarts={echarts} key={`ff-${cluster?.cluster_id}-${quickRange}`} option={buildLineChartOption('', failedFetchRateData, '#f59e0b', '次/秒')} style={{ height: 250 }} notMerge={true} /></div></div> },
          ] : []),
        ]}
      />
      <PromqlDebugger
