...
const ClusterACLs: React.FC = () => {
  ...
  useEffect(() => {
    const loadClusters = async () => {
      try {
        ...
        const cid = searchParams.get('clusterId')
        if (cid && scramClusters.find((c: Cluster) => c.cluster_id === parseInt(cid))) {
          setSelectedClusterId(parseInt(cid))
        } else if (scramClusters.length > 0 && !selectedClusterId) {
  ...
  const handleOpenCreateUser = () => {
    if (!selectedClusterId) { message.warning('请先选择集群'); return }
    createUserForm.setFieldsValue({ cluster_id: selectedClusterId, mechanism: 'SCRAM-SHA-256' })
    setCreateUserVisible(true)
  }

  const handleCreateUser = async (values: any) => {
    try {
      await scramUserService.create({ cluster_id: selectedClusterId!, username: values.username, password: values.password, mechanism: values.mechanism })
      message.success('用户创建成功')
      setCreateUserVisible(false)
      createUserForm.resetFields()
      fetchUsers()
    } catch (error: any) {
      message.error(error?.response?.data?.error || error?.message || '创建用户失败')
    }
  }
    ...
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          ...
          fetchUsers()
        } catch (error: any) { message.error(error?.response?.data?.error || error?.message || '删除失败') }
      },
    })
  }
  ...
  const handleSync = async () => {
    ...
    try {
      ...
      fetchUsers()
    } catch (error: any) { message.error(error?.response?.data?.error || error?.message || '同步失败') }
  ...
  const handleOpenAclModal = (username: string) => {
    ...
    aclForm.setFieldsValue({ cluster_id: selectedClusterId, principal: `User:${username}` })
  ...
  const handleViewAcls = async (username: string) => {
    ...
    try {
      const acls = await getUserACLsFromKafka(selectedClusterId!, `User:${username}`)
      setUserAcls(acls)
    } catch (error: any) { message.error('获取权限列表失败'); setUserAcls([]) }
    finally { setViewAclLoading(false) }
  }

  const handleToggleExpand = async (username: string) => {
    if (expandedUser === username) { setExpandedUser(null); setExpandedAcls([]); return }
    ...
    } catch { /* inline expand silently fails */ }
    finally { setExpandedAclsLoading(false) }
  }
      ...
      onOk: async () => {
        try {
          await deleteACLFromKafka(selectedClusterId!, {
            resource_type: acl.resource_type, resource_name: acl.resource_name,
            resource_pattern: acl.resource_pattern || 'literal', principal: acl.principal,
            host: acl.host || '*', operation: acl.operation, permission_type: acl.permission_type,
          })
          message.success('删除成功')
          const acls = await getUserACLsFromKafka(selectedClusterId!, `User:${viewAclUsername}`)
          setUserAcls(acls)
        } catch (error: any) { message.error(error?.response?.data?.error || error?.message || '删除失败') }
      },
    })
  ...
  const handleCreateAcl = async (values: any) => {
    ...
    try {
      ...
      for (const op of operations) {
        await createACL({
          cluster_id: selectedClusterId, resource_type: values.resource_type,
          resource_name: values.resource_name, resource_pattern: values.resource_pattern || 'literal',
          principal: values.principal, operation: op, permission_type: values.permission,
        })
      }
      message.success(`成功设置 ${operations.length} 条权限规则`)
      setAclModalVisible(false)
      aclForm.resetFields()
    } catch (error: any) { message.error(error?.response?.data?.error || error?.message || '权限设置失败') }
  }
  ...
  const filteredUsers = users.filter(u => !searchText || u.username.toLowerCase().includes(searchText.toLowerCase()))
  ...
  return (
    <div>
      <div className="page-header">
        <div className="page-header-row">
          ...
          <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
            <Select showSearch placeholder="选择集群" value={selectedClusterId}
              onChange={(value) => setSelectedClusterId(value)} style={{ width: 200 }}
              optionFilterProp="children" filterOption={(input, option) => (option?.children ?? '').toString().toLowerCase().includes(input.toLowerCase())}>
            ...
            <Button type="primary" icon={<PlusOutlined />} onClick={handleOpenCreateUser} disabled={!selectedClusterId}>创建用户</Button>
          </div>
        </div>
      </div>
          ...
          <div className="bento-table-body">
            ...
            {!loading && filteredUsers.map((user, idx) => (
              <div key={user.user_id} style={{ borderBottom: '1px solid var(--border)' }}>
                <div className="bento-table-row" style={{ gridTemplateColumns: gridCols, cursor: 'pointer' }} onClick={() => handleToggleExpand(user.username)}>
                  ...
                  <div style={{ textAlign: 'center' }}><LabelTag text={user.mechanism} color={user.mechanism === 'SCRAM-SHA-256' ? 'purple' : 'blue'} /></div>
                  ...
                  <div style={{ textAlign: 'right', display: 'flex', gap: 6, justifyContent: 'flex-end' }}>
                    <button className="bento-action-btn" onClick={(e) => { e.stopPropagation(); handleViewAcls(user.username) }}><EyeOutlined /> ACL</button>
                    <button className="bento-action-btn bento-action-btn--primary" onClick={(e) => { e.stopPropagation(); handleOpenAclModal(user.username) }}><KeyOutlined /> + Rule</button>
                    <button className="bento-action-btn bento-action-btn--danger" onClick={(e) => { e.stopPropagation(); handleDeleteUser(user.username) }}><DeleteOutlined /> 删除</button>
                  </div>
                </div>
                {expandedUser === user.username && (
                  <div style={{ padding: '0 24px 16px' }}>
                    <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, padding: 16 }}>
                      <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-3)', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 12 }}>ACL Rules for {user.username}</div>
                      {expandedAclsLoading && <div style={{ fontSize: 12, color: 'var(--text-3)' }}>加载中...</div>}
                      {!expandedAclsLoading && expandedAcls.length === 0 && <div style={{ fontSize: 12, color: 'var(--text-3)' }}>暂无权限规则</div>}
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                        {expandedAcls.map((acl, i) => (
                          <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 12px', background: 'var(--card)', borderRadius: 8, border: acl.permission_type === 'Deny' ? '1px solid rgba(239,68,68,0.15)' : 'none' }}>
                            ...
                            <button className="bento-action-btn bento-action-btn--danger" style={{ fontSize: 10, padding: '2px 8px' }} onClick={() => handleDeleteAcl(acl)}>删除</button>
                          </div>
                        ))}
                      </div>
                    </div>
            ...
            {!loading && filteredUsers.length === 0 && (
              <div style={{ textAlign: 'center', padding: 48, color: 'var(--text-3)' }}>{selectedClusterId ? '暂无用户数据' : '请先选择集群'}</div>
            )}
          </div>
        </>
      )}

      <Modal title="创建 SCRAM 用户" open={createUserVisible} onCancel={() => { setCreateUserVisible(false); createUserForm.resetFields() }} onOk={() => createUserForm.submit()}>
        <Form form={createUserForm} layout="vertical" onFinish={handleCreateUser}>
          <Form.Item name="cluster_id" label="集群" rules={[{ required: true }]}>
            <Select placeholder="选择集群" disabled>{clusters.map(c => (<Select.Option key={c.cluster_id} value={c.cluster_id}>{c.cluster_name}</Select.Option>))}</Select>
          </Form.Item>
          <Form.Item name="username" label="用户名" rules={[{ required: true, message: '请输入用户名' }]}><Input placeholder="请输入用户名" /></Form.Item>
      ...
      <Modal title={`用户权限 - ${viewAclUsername}`} open={viewAclVisible} onCancel={() => { setViewAclVisible(false); setUserAcls([]) }} footer={[<Button key="close" onClick={() => setViewAclVisible(false)}>关闭</Button>]} width={850}>
        <div className="bento-table-header" style={{ gridTemplateColumns: '90px minmax(130px, 1.5fr) 70px 100px 70px 100px 70px' }}>
          <div>资源类型</div><div>资源名称</div><div>匹配</div><div>操作</div><div>权限</div><div>Host</div><div style={{ textAlign: 'right' }}>操作</div>
        </div>
        <div className="bento-table-body">
          ...
          {!viewAclLoading && userAcls.map((acl, index) => (
            <div key={`${acl.resource_type}-${acl.resource_name}-${acl.operation}-${index}`} className="bento-table-row" style={{ gridTemplateColumns: '90px minmax(130px, 1.5fr) 70px 100px 70px 100px 70px' }}>
              ...
              <div style={{ textAlign: 'right' }}><button className="bento-action-btn bento-action-btn--danger" style={{ fontSize: 10 }} onClick={() => handleDeleteAcl(acl)}>删除</button></div>
            </div>
          ))}
        </div>
      </Modal>

      <Modal title={`权限设置 - ${selectedUsername}`} open={aclModalVisible} onCancel={() => { setAclModalVisible(false); aclForm.resetFields() }} onOk={() => aclForm.submit()} width={600}>
        <Form form={aclForm} layout="vertical" onFinish={handleCreateAcl}>
          <Form.Item name="principal" label="Principal"><Input disabled /></Form.Item>
          ...
          <Form.Item name="resource_name" label="资源名称" rules={[{ required: true, message: '请输入资源名称' }]}>
            <Input placeholder={selectedResourceType === 'Cluster' ? '集群资源名称必须填 kafka-cluster' : '资源名称（如 test-topic 或 * 表示所有）'} />
          </Form.Item>
          ...
          <Form.Item name="operation" label="操作" rules={[{ required: true, message: '请选择操作' }]}>
            <Select placeholder={selectedResourceType ? '选择操作' : '请先选择资源类型'} mode="multiple" disabled={!selectedResourceType}>
              {(RESOURCE_OPERATIONS[selectedResourceType] || []).map(op => (<Select.Option key={op.value} value={op.value}>{op.label}</Select.Option>))}
            </Select>
          </Form.Item>
